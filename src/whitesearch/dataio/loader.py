...
from .gw_observation import prepare_gw_from_record, prepare_gw_from_simdata
...
def load_observation_data(
    ...
    *,
    ...
    if source == "mock":
        data = _load_mock(channel, inject_model, context, seed, reference_amplitude)
        prov = DataProvenance(
    ...
    if source == "gwosc":
        ...
        try:
            ...
            record = prepare_gw_from_record(
                bundle[det], reference_amplitude=reference_amplitude
            )
            ...
            data = _load_mock(
                channel, inject_model, context, seed, reference_amplitude
            )
            prov = DataProvenance(
                requested_source="gwosc",
...
def _load_mock(
    ...
    ctx = dict(context)
    ctx["rng_seed"] = seed
    sim_data = sim.simulate(params, ctx, rng=np.random.default_rng(seed))
    if channel == "gw":
        return prepare_gw_from_simdata(
            sim_data, reference_amplitude=reference_amplitude
        )
    return sim_data
