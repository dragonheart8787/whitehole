...
| `docs/XRAY_IMAGE_PREFLIGHT_AUDIT.md` | xray + image 通道稽核（X.0–X.16） |
...
**成熟度：forward model 與先驗皆已到位，取樣成本已降到可負擔範圍。**
仍**沒有任何 coverage 證據**——完整規模 campaign 尚未開跑，見 I-6。
...
| 項目 | 分類 |
|---|---|
| SBC 取樣成本：原始參數化下 N=100 約 145 小時。**改為總流量參數化後降到約 11.9 小時**（中位單筆 7.1 分），但這是下界——N=6 的 pilot 有 2 筆未收斂 | **已處理，待你決定是否開跑**（見 I-6） |
...
- **要決定的**：(a) 是否開跑完整規模 campaign（N=100 約 11.9 h 起跳，
  在這個容器只能以 ≤ 600 s 前景片段推進）；(b) 是否處理上列 1–4。
- **分類**：**需要你做一個決定**。
...
- **已量測**（X.16）：改為總流量參數化後，M87\* 的四個收斂點是
  SNR 4.8 → 64 s、31.3 → 256 s、73.5 → 552 s、149.2 → 627 s。
  同 SNR 對照舊參數化快 **3.9–5.3 倍**。
- **估計**：擬合 `t = -236 + 171 ln(SNR)` 套用新先驗的 SNR 分布，
  單筆 5/50/95% = 30 / **427** / 849 s，**N=100 約 11.9 小時**、N=20 約 2.4 小時。
- **保留意見**：只有 4 個收斂點、散布大（SNR 201.2 跑 1438 s 仍未收斂），
  **11.9 h 是下界**；只跑了 M87\*，Sgr A\* 更亮、成本更高。
