...
一致性都已被排除（I-6d、I-6e、I-6g、I-6h），六參數均勻偏窄**沒有候選解釋**，見 I-6f。
...
### I-6k｜image｜`f_pix(θ)` 階梯狀跳動 —— **未測試，已識別但尚未驗證**
...
- **分類**：**未測試。** 這是目前候選清單上唯一還開著的項目。
- **細節**：`docs/XRAY_IMAGE_PREFLIGHT_AUDIT.md` X.26。
...
| `f_pix(θ)` 階梯狀跳動扭曲真值附近曲面形狀 | **未測試，已識別但尚未驗證**（I-6k） | 未知 |
...
- **分類**：**需要你決定下一步**。候選清單要重想，本輪依指示未自行往下猜。
  ...
  目前唯一還開著的是 **I-6k**（`f_pix(θ)` 階梯效應），且尚未驗證。
