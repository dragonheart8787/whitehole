...
| `docs/XRAY_IMAGE_PREFLIGHT_AUDIT.md` | xray + image 通道稽核（X.0–X.17） |
...
**成熟度：forward model 與先驗皆已到位，N=20 中間規模驗證已完成。**
**尚無正式 coverage 判定**——N=19 的初步統計只是方向性觀察，見 I-6b。
...
| 項目 | 分類 |
|---|---|
| SBC 取樣成本：原始參數化下 N=100 約 145 小時 → 改總流量參數化後 **N=20 實測 2.23 h、收斂率 95%、N=100 外推 11.2 h** | **已驗證可行，待你決定是否續跑到 N=100**（見 I-6、I-6b） |
| N=19 的初步 rank/coverage：KS 全數通過 Bonferroni，但 `log10_total_flux_jy` rank 偏高（66.5 vs 50）且 coverage 系統性偏低（`i` 的 90% 是 0.579 vs 理論 0.891） | **方向性觀察，樣本數不足以判定**（見 I-6b） |
...
### I-6b｜image｜N=20 中間規模驗證 —— **路徑可行，N=100 約 11.2 h**

- **設定**：與 X.16 的 pilot 完全相同（`gr_eternal`、M87\*、6 維、
  `use_closure_phases=False`、`nlive=250`、`nact=2`、
  `log10_total_flux_jy` 參數化）。pilot 的 6 筆種子一致，直接併入未重跑。
  **L = 100**，逐筆記錄 `n_posterior`（最少 478）確認分母沒有失真。
  單筆上限 1500 s——為此把 harness 改成跨 shard 累計 `cum_wall_s`
  （`_BudgetGuard` 的預算是每個 shard 的，resume 後會重新計時）。
  **只改 harness，沒有動 production 程式碼。**
- **收斂率 19/20 = 95%**。唯一未收斂的是 idx 2（SNR 201.2）用滿 1500 s。
  19 筆收斂的牆鐘 5/50/95% = 49 / 224 / 924 s，平均 344 s。
  **N=20 實際花 2.23 h。**
- **耗時模型與 pilot 幾乎相同**：`t = -242.8 + 167.1 ln(SNR)`（19 點）
  對上 pilot 的 `t = -236.0 + 171.0 ln(SNR)`（4 點）。
  **pilot 在高 SNR 端「失準」的疑慮沒有成立**——最高 SNR 的那筆（585.7）
  實測 863.9 s 對擬合 822 s，貼合得很好。
- **真正的問題是散布，不是趨勢**：R² = 0.584、殘差 rms 231 s。
  idx 3 與 idx 10 的 SNR（149.2 / 150.1）與真值幾乎相同，耗時卻差 2.3 倍
  （627 s vs 1467 s）。對耗時殘差與每個真值參數做 Spearman，
  **沒有任何一個顯著**（最強的 `position_angle` ρ=+0.416, p=0.077）。
  判定為**取樣器 run-to-run 變異**，與先前記錄的「dynesty 固定種子仍不可重現」
  一致。意思是：**用平均外推 N=100 可靠，逐筆預測不可靠。**
- **N=100 估計**：pilot 擬合 11.93 h、N=20 擬合 11.35 h、
  這 20 筆實測平均外推 **11.15 h**——三者一致，與原本的 11.9 h 沒有明顯差異。
  **Sgr A\*（未測）** 先驗中位 SNR 126.5（M87\* 是 48.8），外推 **約 17.8 h**。
- **初步 rank / coverage（N=19，不是判定）**：六個參數的 KS p 值
  **全部通過 Bonferroni 門檻**（p > 0.0083）。方向性觀察兩項：
  (a) `log10_total_flux_jy` rank 平均 66.5（理論 50）、KS p = 0.017，
  rank 偏向高端，代表後驗相對真值偏低；
  (b) coverage 系統性偏低，90% 有五個參數低於理論值 0.891，`i` 的 0.579
  低了 4.4σ。兩項在 N=19 下都還在噪聲範圍，但方向一致。
  **coverage 的理論值是 0.673 / 0.891 而非 0.68 / 0.90**——L=100 個抽樣的
  經驗分位數區間覆蓋新抽樣的機率是 `level × L/(L+1)`。
  這 19 筆的 rank 與 CI 已存檔，跑到 N=100 可直接併入不需重跑。
- **建議**：路徑可行。建議跑到 N=100 之前先確認上述 (a)(b) 是否在更大樣本下持續。
- **細節**：`docs/XRAY_IMAGE_PREFLIGHT_AUDIT.md` X.17。原始數字：
  `docs/calibration/image_gr_eternal_n20_injections.csv`、
  `image_gr_eternal_n20_rank_coverage.csv`。
...
- **已實測**（X.17）：M87\* 的 N=20 花 **2.23 h**，收斂率 **19/20 = 95%**，
  單筆牆鐘 5/50/95% = 49 / 224 / 924 s。
- **耗時模型**：`t = -242.8 + 167.1 ln(SNR)`（19 點，R² = 0.584）。
  與 pilot 的 4 點擬合幾乎相同；殘差散布（rms 231 s）判定為取樣器變異，
  與任何真值參數都不顯著相關。
- **N=100 外推**：**約 11.2 h**（M87\*）。Sgr A\* 未測，外推約 17.8 h。
- **已完成的 19 筆 rank/CI 已存檔**，續跑到 N=100 可直接併入。
- **執行環境限制仍在**：容器在 turn 之間暫停，只能以 ≤ 600 s 的前景片段推進
  （N=20 實際用了約 12 個 shard 呼叫）。
