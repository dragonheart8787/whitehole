...
模型樣板修正後大幅改善；(b) 後驗偏窄已被 nact pilot 排除（I-6d），
剩下 (c) 振幅機率模型與一個尚無名字的 under-coverage 來源，見 I-6c / I-6d。
...
| 項目 | 分類 |
|---|---|
| **N=100 完整校準跑過兩輪**：修正模型樣板後（I-6c）四個參數的 rank 問題解決、coverage 平均 0.800 → 0.839（理論 0.891），但仍未通過；候選 (b)（`nact` 鏈長）已被 18 筆定向 pilot 排除（效應 0.0% ± 2.4%，缺口需 +17.3%） | **阻塞性，殘留剩 (c) 與一個未定性的來源**（見 I-6b、I-6c、I-6d） |
