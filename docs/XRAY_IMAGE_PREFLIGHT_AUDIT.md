...
|---|---|---|
| 0.839（X.20 全 95 筆，90%） | 0.90 | **+17.3%** |
| 0.614（X.20 全 95 筆，68%） | 0.68 | +14.7% |

**需要 +17.3%，量到的是 0.0%（±2.4%）。**
...
而補上 0.05 的 coverage 缺口需要 +17.3% 的增寬——
...
| 觀測 coverage | 名目 | 需要的區間放寬 |
|---|---|---|
| 0.839（X.20 全 95 筆） | 0.90 | **+17.3%** |
| 0.806（本 18 筆子集） | 0.90 | +26.6% |

**需要 +17.3%，量到 −1.1%（95% 上界 +2.0%）。**
...
沿用 X.21.6 的算法（`2Φ(1.645/k) − 1`）：補上 0.839 → 0.891 需要 **+17.3%**。
...
**三個旋鈕的 95% 上界合起來是 +0.6% 到 +2.4%，全部遠低於 +17.3%。**
...
方向與需求相反），而補上 0.05 的 coverage 缺口需要 +17.3%；
