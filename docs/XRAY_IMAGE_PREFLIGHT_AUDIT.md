...
# X.29 收斂選擇效應對殘留 coverage 缺口的影響評估

X.28.6 列出的限制之一：X.20 起跑 100 筆、分析的是收斂的 95 筆，
被排除的 5 筆 `timeout_capped` 的 SNR（583.5 / 349.3 / 791.9 / 572.8 / 140.4）
**全部在收斂集的上四分位（149.7）之上**，所以分析集不是先驗空間的均勻抽樣。
本節檢驗這件事會不會讓 +11.7% 的缺口失真。
**純粹是對既有資料的統計分析，沒有跑任何新的取樣。**

## X.29.1 兩個設計要點

1. **選擇變數是「成本」，不是 SNR。** 那 5 筆被排除是因為超過 1500 s 的
   wall-clock 上限，SNR 只是與它相關的代理變數。只對 SNR 迴歸會檢到代理，
   檢不到機制，**所以兩個都跑**。
2. **所有推論都對「注入」重抽**，不對 6 × N 個指標重抽——
   同一筆注入的六個指標共用一個後驗（沿用 X.28.1 的理由）。

## X.29.2 coverage 與 SNR：完全平坦

| | 90% | 68% |
|---|---|---|
| logistic 斜率（每 dex） | **+0.0332** | **+0.0473** |
| 95% CI（叢集 bootstrap） | [−0.367, +0.429] | [−0.198, +0.294] |
| p | **0.897** | **0.698** |
| SNR 三分位 coverage | 0.8333 / 0.8387 / **0.8438** | 0.5885 / 0.6613 / 0.5938 |
| 上下半比較（高−低） | +0.0174，CI [−0.046, +0.081] | +0.0362，CI [−0.037, +0.110] |

**在 2.25 dex 的 SNR 範圍上，coverage 是平的。**
特別值得記下來的是：**最亮的三分位 coverage 是 0.8438，
仍然遠低於參考值 0.8825**——缺口在**每一個** SNR 區間都在，
不是暗源端的產物。

（對照 X.19/X.20 的 (a) 修正：那次的效應**強烈依賴 SNR**
（暗的一半 +13.0%、亮的一半 +0.11%），所以這裡看到的「平坦」
不是方法不敏感，同一套分層方法在真有 SNR 依賴時是抓得到的。）

## X.29.3 coverage 與成本（真正的選擇變數）：同樣沒有顯著關聯

| | 90% | 68% |
|---|---|---|
| logistic 斜率 vs `log10(cum_wall_s)` | **+0.219** | **+0.145** |
| 95% CI | [−0.362, +0.798] | [−0.235, +0.537] |
| p | **0.463** | **0.443** |
| 成本三分位（42–102 / 104–363 / 369–1435 s） | 0.8281 / 0.8333 / **0.8542** | 0.5938 / 0.6290 / 0.6198 |
| 最接近上限的（≥1000 s，n = 4） | **0.9167**（其餘 0.8352） | 0.6667（其餘 0.6117） |

不顯著，而且**點估計的方向對「偏差疑慮」是相反的**：
越貴的注入 coverage 略**高**。如果這個趨勢延續到被排除的那 5 筆，
它們會把 coverage 往**上**推一點點——也就是縮小缺口，但幅度很小（見下）。

## X.29.4 外推：不管那 5 筆是什麼結果，能動的幅度都很小

| 假設那 5 筆的 coverage 是 | 合併後 coverage | 位移 | 所需增寬 |
|---|---|---|---|
| 高成本三分位的 0.8542 | 0.8394 | **+0.0008** | +11.6% |
| 最接近上限那 4 筆的 0.9167 | 0.8425 | **+0.0039** | +10.7% |
| **上界：5 筆全部完全被涵蓋（30/30）** | 0.8467 | **+0.0081** | **+9.6%** |
| 下界：5 筆全部完全沒被涵蓋（0/30） | 0.7967 | −0.0419 | +23.1% |

（原值：0.8386、缺口 +0.0439、所需增寬 +11.8%。）

**最關鍵的數字不是任何一條迴歸，而是一個算術上界**：
100 筆裡只缺 5 筆，所以無論那 5 筆是什麼結果，
合併 coverage 最多只能往上移

```
5/100 × (1 − 0.8386) = 0.0081
```

也就是 **0.0439 缺口的 18%**——而且那需要**五筆全部完美涵蓋**，
是上界不是估計。68% 那一側對應的上界是 36%（0.0193 / 0.0531），
同樣需要五筆全部完美涵蓋。

即使在那個極端情形下，所需增寬仍有 **+9.6%**（90%）與 **+7.3%**（68%），
仍然遠高於三個取樣器 pilot 的 95% 上界（+2.4% / +2.0% / +0.6%）。

## X.29.5 一句話結論

**收斂選擇效應不是缺口的主因**：coverage 與 SNR（p = 0.897）
與成本（p = 0.463）都沒有顯著關聯，缺口在每一個 SNR 與成本三分位裡
都存在（最亮三分位 0.8438、最貴三分位 0.8542，都遠低於 0.8825）；
以現有資料外推，納入那 5 筆只會讓 coverage 移動 **+0.0008 到 +0.0039**，
而**算術上界是 +0.0081，即缺口的 18%**——量級上不足以構成重要影響。

## X.29.6 要不要把那 5 筆重跑到收斂（回報，未執行）

**技術上可行，但依 X.29.4 的上界，不建議為了這個缺口去跑。**
供你判斷的數字：

- 那 5 筆在 1500 s 的上限下未收斂，所以真實成本 **> 1500 s**；
  收斂集裡最貴的一筆是 1435 s。
- 放寬到 6000 s 上限（比照 X.21 的做法），**最壞情況 5 × 6000 s ≈ 8.3 小時**，
  以收斂集的成本分布外推，實際大概 **3–6 小時**。
- 它能買到的：把上表那一列從「外推」換成「實測」。
  但即使實測結果落在最有利的極端，缺口也只從 +0.0439 降到 +0.0358。

**依指示未開跑。**

## X.29.7 I-6 這條調查線目前最誠實的狀態

| 面向 | 狀態 |
|---|---|
| **(a) 模型樣板誤用含雜訊資料** | **已修正**，coverage 0.800 → 0.839，是真實的改進，但沒補完 |
| **七個候選機制** | **全部排除**：`nact`、`nlive`、`dlogz`、`sigma` 一致性、(c) 的均值與變異數、SBC/coverage 計算機制、`n_pixels`、`f_pix(θ)` 階梯效應 |
| **缺口的統計顯著性** | **顯著**（合併 z = −2.70 / −2.84，bootstrap p = 0.0035 / 0.0019，兩個信賴水準同方向同量級） |
| **逐參數顯著性** | **沒有任何單一參數單獨顯著**（Bonferroni 後全部不顯著）——「全域、均勻、小幅」效應該有的樣子 |
| **收斂選擇效應** | **不是主因**，算術上界 18%，實際外推 2–9% |
| **效應量** | 所需區間增寬 **+11.8%**（95% CI 3.3–20.2%） |

**目前所有已知可檢驗的解釋都已窮盡，缺口的成因未知。**
需要一個全新的角度，或是接受這是 image 通道目前**已知但未解的限制**。
...
- `scripts/test_convergence_selection.py`

---

# X.30 `position_angle` 週期性／環繞效應對 rank 計算影響的檢查

`position_angle` 的單獨 p 值（0.0139）是六個參數裡最低的一個
（Bonferroni 後不顯著）。假說：如果它的先驗支撐是週期性的，而 rank 與
信賴區間的計算都當成線性量處理，當真值落在邊界附近、後驗樣本跨越邊界
分裂成兩群時，算出來的區間會系統性失真。
**純程式碼閱讀＋對既有存檔資料的數值檢查，沒有跑任何取樣，沒有改動程式碼。**

## X.30.1 前提確認：它**確實**是週期性的，而且計算**確實**沒有做環繞

**先驗**：`Uniform(0, π)`，宣告方式與其他線性參數完全相同
（`prior_type="uniform"`），**沒有任何「週期性／循環」的標記**。

**物理週期是 π，不是 2π**——這是從 `_gaussian_ring_image()` 的實作確認的，
不是假設的。`gr_eternal` 的 `asym_amp = 0.0`（軸對稱環），影像是

```
r_ellipse = sqrt(xr² + (yr / axial_ratio)²)
image     = brightness * exp(-0.5 ((r_ellipse - r_ring) / w_ring)²)
```

旋轉 π 把 `(xr, yr)` 映到 `(−xr, −yr)`，`r_ellipse` 完全不變。實測：

| 檢查 | 結果 |
|---|---|
| `max|I(PA) − I(PA+π)|`（五個 PA 值） | **9.5×10⁻²⁰ – 2.5×10⁻¹⁹**（浮點層級） |
| `max|I(PA) − I(PA+π/2)|` | 7.5×10⁻⁵（真的不同） |
| `max|I(0) − I(π)|` | **9.5×10⁻²⁰** |

**先驗支撐剛好是一個完整週期，而且兩個端點 0 與 π 是同一個模型。**
這是一個不折不扣的圓周變數。

（順帶記錄的物理事實：`axial_ratio = |cos i|`，所以接近正面
（`i → 0`）時橢圓退化成圓，`position_angle` 幾乎無法辨識——
實測 `i = 0.05 rad` 時 PA 從 0.2 變到 1.2 只讓影像變 4.1×10⁻⁷。
這是真實的簡併，不是 bug。）

**計算端**：`compute_sbc_rank` 是 `np.sum(posterior_samples < true_value)`，
`compute_credible_interval` 是 `np.quantile(samples, α)` 與 `1−α`。
**兩者對所有參數一視同仁地當線性量處理，全樹沒有任何週期性／環繞的特殊處理。**

**所以假說的前提兩項都成立。**

## X.30.2 存檔資料裡確實有大量跨邊界分裂

用 X.20 存檔的 95 筆重建當初那組 `L = 100` 的抽稀樣本
（抽稀是確定性的；重算 16 個 rank 與存檔值 **0 個不一致**，確認重建正確）：

| 量 | 值 |
|---|---|
| 樣本同時出現在先驗底部 10% 與頂部 10% | **48/95（50.5%）** |
| 其中圓周上**集中**的（`R > 0.5`，真正的跨邊界單群） | **19/95（20.0%）** |
| 線性 90% 區間寬度中位數 | 1.0624 rad（先驗的 33.8%） |
| 圓周最小弧 90% 寬度中位數 | 0.9248 rad |
| **線性／圓周寬度比** | 中位數 **1.078**；> 1.2 的有 **30/95**；最大 **14.91** |

**線性區間確實被環繞撐寬了**，中位數多 7.8%，最糟的一筆多近 15 倍。

真值落在邊界附近的比例與均勻先驗的期望相符
（距邊界 0.1 rad 內 8 筆 vs 期望 6.4%），沒有異常聚集。

## X.30.3 但環繞**不會**造成 under-coverage——方向相反，而且理論上不可能

**理論**：SBC 的 rank 均勻性來自**可交換性**（exchangeability）——
真值與 L 個後驗抽樣在正確校準下是同分布可交換的。可交換性對**任何固定的
全序**都成立，**不要求那個序尊重參數的拓撲**。而等尾分位數區間的
coverage 本質上是 rank 的函數，所以它繼承同一個保證。
**環繞只會讓區間比必要的更寬，不會讓它失準。**

**模擬驗證**（不是只講道理）：造一個週期 π 的圓周參數，後驗**完美校準**，
用本專案的線性 rank 與等尾區間去評分，N = 100000：

| 後驗寬度 sd | 樣本跨邊界的比例 | rank 卡方 p（101 格） | cov68 | cov90 | rank 平均 |
|---|---|---|---|---|---|
| 0.02 | 0.033 | — | 0.6686 | 0.8821 | 50.03 |
| 0.05 | 0.080 | — | 0.6648 | 0.8808 | 50.10 |
| 0.15 | 0.241 | 0.3992 | 0.6670 | 0.8826 | 49.90 |
| 0.40 | 0.623 | — | 0.6686 | 0.8833 | 50.05 |
| 0.80 | **0.997** | 0.5345 | 0.6664 | 0.8810 | 50.00 |
| **參考值（X.25）** | — | — | **0.6671** | **0.8825** | **50.0** |

**即使 99.7% 的樣本都跨邊界，coverage 仍然精確命中參考值。**

**一個必須說明的陷阱**：這些模擬的 KS p 值看起來很糟（10⁻⁵ 量級），
但那是**把連續的 KS 檢定套在離散 rank（101 個值）上、N = 100000 時的
已知假象**——跑一個**沒有任何環繞的線性對照組**，KS p 同樣是 10⁻⁵ 量級
（0.00000 / 0.00001），而 101 格的卡方檢定在**所有**情形（圓周與線性）
都通過（p = 0.36–0.60）。差點被這個假象誤導，記錄於此。

## X.30.4 存檔資料也看不到關聯

| 分組 | hit / miss（90%） | 其餘 | Fisher p |
|---|---|---|---|
| 跨邊界且圓周集中（19 筆） | 14 / 5 | 62 / 14 | **0.5225** |
| 線性／圓周寬度比 > 1.05（60 筆） | 50 / 10 | 26 / 9 | **0.3007** |

**沒有關聯。**

而且**方向本來就相反**：環繞讓區間變**寬**，若有影響應該是 **over**-coverage。
但 `position_angle` 的 coverage 是六個裡**最低**的（90% 0.800、68% 0.547）。
**環繞效應與觀察到的現象方向相反，不能解釋它，反而讓它更難解釋。**

## X.30.5 一句話結論

**假說的前提兩項都成立——`position_angle` 確實是週期 π 的圓周變數
（實測 `max|I(PA)−I(PA+π)| ≈ 10⁻¹⁹`，先驗剛好是一個完整週期、端點同一個模型），
rank 與信賴區間確實完全沒有做環繞處理，而且存檔資料裡有 50.5% 的注入
樣本跨邊界、線性區間中位數被撐寬 7.8%（最糟 14.9 倍）——
但這個效應在理論上（可交換性）與模擬上（99.7% 跨邊界時 coverage 仍是
0.8810 對上參考 0.8825）都**不會造成 under-coverage**，
存檔資料也看不到關聯（Fisher p = 0.52 / 0.30），
方向更是相反（環繞只會讓區間偏寬）——**這個候選排除。**

## X.30.6 一併記錄的發現（不是 bug，未處理）

**`position_angle` 的信賴區間比它需要的寬**：中位數多 7.8%，
30/95 筆多超過 20%，最糟一筆多 14.9 倍。這**不影響校準**（X.30.3），
但讓這個參數的區間**比實際資訊量所允許的更不精確**。
要不要改成圓周統計（最小弧／圓周分位數）是一個獨立的設計決定，
**依指示未動手，先回報。**

同樣未處理的還有 X.25.5 記錄的 `compute_credible_interval` docstring
寫 "highest posterior density" 但實作是等尾分位數。

## X.30.7 產出

- `docs/calibration/image_gr_eternal_position_angle_wrap.csv`
- `scripts/check_position_angle_wrap.py`

---

# X.31 `gr_eternal` 在 Sgr A\* 上的 SBC/coverage 校準與跨目標比較

X.18 – X.30 的每一個數字都來自 M87\*（postmortem 的「附帶發現未處理」第 7 項）。
本節在 Sgr A\* 上跑同一條 pipeline：已修正的 (a) likelihood、
`log10_total_flux_jy` 參數化、`use_closure_phases=False`、
`nact = 2` / `nlive = 250` / `dlogz = 0.1`，網格與先驗依 I-3 不動。

## X.31.1 先驗預測差異（300 次抽樣，未跑取樣）

| | M87\* | Sgr A\* |
|---|---|---|
| 不可表示比例 | 0% | **0%** |
| 環半徑 μas 5/50/95% | 9.4 / 16.4 / 27.7 | **22.0 / 25.8 / 30.2** |
| 總流量 Jy 5/50/95% | 0.066 / 0.718 / 9.32 | 0.255 / 2.09 / 20.0 |
| 網路 SNR 5/50/95% | 4.1 / 45.9 / 583.6 | **14.1 / 118.6 / 1118.5** |
| SNR ≥ 8 的比例 | 83.3% | **100%** |
| SNR ≥ 1000 的比例 | 0.0% | **6.7%** |

Sgr A\* 的環半徑範圍窄得多，因為它的質量先驗只有 0.155 dex
（M87\* 是 0.52 dex）；SNR 則整體高約 2.6 倍。
**兩者都是 0% 不可表示**，I-3 的結論在第二個目標上成立。

## X.31.2 pilot 與成本

12 筆依 SNR 分層抽樣的 pilot **12/12 收斂**，總 1.36 h。
擬合 `log10(wall_s) = 1.304 + 0.560 × log10(SNR)`（殘差散布 1.37×），
外推 N = 100 約 **10.3 h**（分層平均法交叉檢查 11.3 h），
落在 M87\* 的 10.23 h 量級內，因此直接開跑完整規模。

**實際 12.72 h，比外推高 23%。** 差異來自 SNR 尾端：
成本模型預測 0 筆會被 3000 s 上限截斷，實際有 1 筆
（`idx 25`，SNR 1118.3）。**冪律模型低估了高 SNR 尾端的成本**，記錄備查。
...
|---|---|---|
| 起跑 / 收斂 | 100 / **99** | 100 / 95 |
| `timeout_capped` | **1**（SNR 1118.3） | 5（SNR 140–792） |
| `UnrepresentableRingError` | **0** | 0 |
| 耗時 | 12.72 h | 10.23 h |
| 上限 | 3000 s | 1500 s |

Sgr A\* 的收斂率更高（99% vs 95%），**收斂選擇效應的分母問題比 M87\* 更小**。

## X.31.4 Sgr A\* 六參數 rank / coverage（N = 99）

| 參數 | rank 平均 | rank sd | KS p | 68% cov | 90% cov |
|---|---|---|---|---|---|
| `M` | 46.9 | 28.4 | 0.2520 | 0.677 | 0.879 |
| `a_star` | 43.3 | 30.4 | 0.0710 | 0.596 | 0.798 |
| `i` | 47.2 | 28.3 | 0.0733 | 0.667 | 0.909 |
| `position_angle` | 54.7 | 31.8 | 0.0573 | 0.616 | 0.788 |
| `ring_width_frac` | 52.9 | 29.0 | 0.2708 | 0.626 | 0.909 |
| `log10_total_flux_jy` | 46.6 | 28.4 | 0.1253 | 0.677 | 0.869 |
| **參考值** | **50.0** | **29.2** | — | **0.667** | **0.882** |

**沒有任何參數的 KS p 低於 0.05**（最低是 `position_angle` 的 0.0573）。

## X.31.5 並排比較

| 參數 | Sgr A\* 90% cov | M87\* 90% cov | 差 |
|---|---|---|---|
| `M` | 0.879 | 0.863 | +0.016 |
| `a_star` | 0.798 | 0.832 | −0.034 |
| `i` | 0.909 | 0.842 | +0.067 |
| `position_angle` | 0.788 | 0.800 | −0.012 |
| `ring_width_frac` | 0.909 | 0.842 | +0.067 |
| `log10_total_flux_jy` | 0.869 | 0.853 | +0.016 |
| **六參數平均 90%** | **0.859** | **0.839** | **+0.020** |
| **六參數平均 68%** | **0.643** | **0.614** | **+0.029** |

**rank KS p 的對照**（這是兩個目標最明顯的差異）：

| 參數 | Sgr A\* KS p | M87\* KS p | Sgr A\* rank 平均 | M87\* rank 平均 |
|---|---|---|---|---|
| `position_angle` | 0.0573 | **0.0193** | 54.7 | 56.5 |
| **`log10_total_flux_jy`** | **0.1253** | **0.0014** | **46.6** | **39.6** |

**M87\* 上流量 rank 的偏移（rank 平均 39.6、KS p = 0.0014）在 Sgr A\* 上不存在**
（46.6、p = 0.1253）。這與 X.20.5 已經定性過的 (c) 的**已記錄 SNR 依賴**方向
一致——Rician 均值抬高約 `σ²/(2|V|)`，SNR 越高抬高越小，而 Sgr A\* 的 SNR
中位數高 2.6 倍。**這是對既有 (c) 定性的觀察，本節不提出任何新機制。**

## X.31.6 殘留缺口的顯著性（方法同 X.28）

以**注入**為獨立單位，對注入重抽的 bootstrap（200000 次），參考值用 X.25 的
有限樣本修正值。

| | Sgr A\* | M87\* |
|---|---|---|
| n（獨立注入） | 99 | 95 |
| 每筆平均涵蓋參數數（90%） | 5.1515（期望 5.2950） | 5.0316 |
| 叢集穩健 z（90%） | **−1.635** | −2.702 |
| **bootstrap p（90%）** | **0.0489** | **0.00346** |
| bootstrap p（68%） | **0.1073** | 0.00192 |
| **所需區間增寬（90%）** | **6.4%**，CI [0.0%, 14.1%] | **11.8%**，CI [3.3%, 20.2%] |
| 所需區間增寬（68%） | 5.1%，CI [0.0%, 14.0%] | 11.7%，CI [3.4%, 20.8%] |
| 逐參數 Bonferroni 後顯著 | **無** | 無 |

**Sgr A\* 的缺口方向相同、量級約一半，但它自己只是邊緣顯著**
（90% 的 p = 0.0489，68% 的 p = 0.1073 不顯著）。
**單靠 Sgr A\* 不足以建立這個效應。**

## X.31.7 兩個目標的差異本身不顯著

對兩組**每筆注入涵蓋數**做兩樣本 bootstrap（50000 次）：

| | 差（Sgr A\* − M87\*） | 95% CI | 雙側 p |
|---|---|---|---|
| 90% coverage | +0.0200 | [−0.0226, +0.0625] | **0.360** |
| 68% coverage | +0.0291 | [−0.0242, +0.0817] | **0.284** |

**兩個目標在統計上無法區分。** 所需增寬的信賴區間也大幅重疊
（[0.0, 14.1] 與 [3.3, 20.2]）。

## X.31.8 合併兩個目標（194 筆注入）

| | 值 |
|---|---|
| 合併 coverage（90%） | 0.8488 |
| 叢集穩健 z | **−3.090** |
| **bootstrap p** | **0.00115** |
| **所需區間增寬** | **9.1%**，95% CI [3.5%, 14.8%] |
| 68%：z / p / 增寬 | −2.807 / **0.00254** / 8.3%，CI [2.4%, 14.5%] |

合併後**顯著性提高、信賴區間收窄**，所需增寬落在 **9.1%（CI 3.5–14.8%）**。

## X.31.9 一句話結論

**這條 pipeline 在 Sgr A\* 上可行（99/100 收斂、0 不可表示、12.72 h），
殘留 under-coverage 的**方向與 M87\* 相同**、量級約一半
（所需增寬 6.4% 對 11.8%），但 Sgr A\* 自己只是邊緣顯著
（90% p = 0.0489、68% p = 0.1073），而**兩個目標的差異本身不顯著**
（p = 0.360 / 0.284）——合併 194 筆後 z = −3.09、p = 0.00115、
所需增寬 9.1%（CI 3.5–14.8%），所以這比較像是**同一個架構層級的現象
在兩個目標上都出現**，而不是 M87\* 特有；但 Sgr A\* 的邊緣顯著性
意味著這是**補強證據，不是乾淨的確認**。**

**唯一明確不同的是 `log10_total_flux_jy` 的 rank 偏移**：
M87\* 上 KS p = 0.0014、rank 平均 39.6，Sgr A\* 上 p = 0.1253、46.6。
**依指示，本節不對這個差異提出任何新的候選機制。**

## X.31.10 產出

- `artifacts/image_sbc_sgra/SgrAstar/`（100 筆，99 收斂）
...
# X.33 新 uv 覆蓋（28 條基線）下殘留 under-coverage 一致性驗證

X.18 – X.31 的整條 `gr_eternal` 校準調查**全部跑在舊的 16 條基線覆蓋上**。
I-5 修正（X.32）把預設覆蓋換成 8 個台站的真實拓撲（28 條基線）。
在評估 closure phase 的邊際效果之前，必須先確認一件事：
**只是 uv 覆蓋規模改變（完全不涉及 closure），殘留的六參數 under-coverage
會不會不一樣。** 本節只跑 `use_closure_phases=False`。

## X.33.1 設計：配對，唯一的變因是 uv 覆蓋

用**與 X.21 完全相同的那 18 筆**分層樣本。`sample_prior` 只依賴 seed，
與 uv 覆蓋無關，所以**兩組的真值逐筆完全相同**（已實測確認），
改變的只有觀測落在 uv 平面的哪些點。
其餘設定一律不動：`nact = 2`、`nlive = 250`、`dlogz = 0.1`、
已修正的 (a) likelihood、`use_closure_phases=False`。

## X.33.2 成本：比每次呼叫的倍率更高

| | 舊 16 條 | 新 28 條 |
|---|---|---|
| 每次 `loglike` | 1.67 ms | **2.41 ms（1.44×）** |
| 18 筆 SNR 中位數 | 36.4 | 37.4（比值 0.94） |
| 18 筆收斂 | 18/18 | **18/18** |
| 18 筆總耗時 | 1.29 h | **2.80 h** |
| 逐筆耗時比 | — | **中位數 2.08×**（平均 2.39×，範圍 1.10–6.26×） |

**耗時比 2.08× 明顯高於每次呼叫的 1.44×**，代表取樣器在新覆蓋下
還需要更多次呼叫，不只是每次更貴。N = 100 外推約 **15.6 h**。

SNR 中位數幾乎沒變（比值 0.94）——新陣列多了 12 條基線，
但也包含 JCMT–SMA 這種近零長度的基線，所以總 SNR 不是單純變高。

## X.33.3 六參數並排（n = 18 兩組）

| 參數 | rank 新 | rank 舊 | KS p 新 | KS p 舊 | 90% 新 | 90% 舊 | 68% 新 | 68% 舊 |
|---|---|---|---|---|---|---|---|---|
| `M` | 53.0 | 51.8 | 0.4289 | 0.8201 | 0.611 | 0.778 | 0.611 | 0.556 |
| `a_star` | 31.1 | 32.7 | 0.0309 | 0.0625 | 0.778 | 0.722 | 0.500 | 0.500 |
| `i` | 43.3 | 43.8 | 0.1936 | 0.6520 | 0.667 | 0.889 | 0.389 | 0.611 |
| `position_angle` | 61.4 | 53.2 | 0.0459 | 0.2858 | 0.722 | 0.667 | 0.444 | 0.500 |
| `ring_width_frac` | 58.6 | 50.2 | 0.0657 | 0.4119 | 0.944 | 0.889 | 0.778 | 0.611 |
| `log10_total_flux_jy` | 48.1 | 31.7 | 0.2037 | **0.0026** | 1.000 | 0.889 | 0.833 | 0.667 |

`a_star`（0.0309）與 `position_angle`（0.0459）的 KS p 落在 0.05 以下，
但**六個參數做多重比較、n = 18**，Bonferroni 後都不顯著。

## X.33.4 殘留缺口：與舊覆蓋無法區分

**判讀時必須用對基準**：這 18 筆在**舊覆蓋**下的 90% coverage 是 **0.8056**，
比全 95 筆的 0.839 低——這個子集本來就偏低（X.29 記錄過）。
所以正確的配對比較是「新 18 筆 vs 舊 18 筆」，
不是「新 18 筆 vs 全 95 筆的 +11.8%」。

| | 90% | 68% |
|---|---|---|
| 新覆蓋 coverage | 0.7870，95% CI [0.7315, 0.8426] | 0.5926，CI [0.4907, 0.6944] |
| 舊覆蓋（同 18 筆） | 0.8056 | 0.5741 |
| **新 − 舊** | **−0.0185**，95% CI [−0.102, +0.074] | **+0.0185**，CI [−0.102, +0.139] |
| **雙側 p** | **0.753** | **0.821** |
| 所需增寬（新） | **25.7%**，CI [10.7%, 41.5%] | 16.9%，CI [0.0%, 46.7%] |
| 所需增寬（舊，同 18 筆） | 20.6% | 21.6% |
| 參考：全 95 筆舊覆蓋 | 11.8%，CI [3.3%, 20.2%] | 11.7% |

**兩種覆蓋在統計上無法區分**（p = 0.753 / 0.821），
而且兩個信賴水準的差異方向相反（90% 略低、68% 略高），
是雜訊該有的樣子。所需增寬的信賴區間也大幅重疊。

## X.33.5 一句話結論

**在新的 28 條基線覆蓋下，amplitude-only 的殘留 under-coverage
與舊的 16 條覆蓋在同一批注入上無法區分**
（90% 差 −0.0185、p = 0.753；68% 差 +0.0185、p = 0.821，方向相反），
所需增寬 25.7%（CI 10.7–41.5%）對上舊覆蓋同 18 筆的 20.6%，區間大幅重疊——
**I-5 的覆蓋修正沒有動搖 X.18–X.31 這條調查的基礎**；
代價是耗時中位數 2.08 倍（N = 100 外推約 15.6 h）。

## X.33.6 一併記錄的觀察（未追查）

`log10_total_flux_jy` 的 rank 偏移在新覆蓋下**減弱**：
KS p **0.0026 → 0.2037**、rank 平均 **31.7 → 48.1**。
X.31 在 Sgr A\* 上也看到同一個參數的偏移消失。
這兩處都與 X.20.5 已定性的 **(c) 的已記錄 SNR 依賴**方向一致，
**但本節不對它提出任何新機制，也未追查。**

## X.33.7 限制

1. **n = 18，信賴區間很寬**（所需增寬 CI 跨 10.7–41.5%）。
   這個 pilot 能排除「量級完全不同」，**不能**分辨 20% 以內的差異。
2. 沒有跑到完整 N = 100；依要求先回報，由你決定是否擴大規模。
3. **本節完全沒有跑 closure phase**；`use_closure_phases=True` 的兩組對照
   是下一步，依指示未開跑。

## X.33.8 產出

- `artifacts/image_sbc_uv28/M87star/`（18 筆，18/18 收斂）
- `docs/calibration/image_gr_eternal_uv28_consistency.csv`

---

# X.34 closure phase 對 coverage 影響的定向 pilot（第二臂）

**這是校準診斷，不是任何白洞結論。** WhiteSearch 是候選排序引擎。

## X.34.1 設定

X.33 的 amplitude-only 臂已存檔；本節是同一組對照的**第二臂**：
**完全相同的 18 筆分層樣本**（indices
29, 98, 79, 52, 76, 51, 61, 62, 84, 71, 97, 39, 96, 72, 95, 94, 23, 80，
同樣的 seed = 700000 + idx，因此真值與資料逐筆相同）、
同樣的新 28 條基線台站陣列覆蓋、
唯一改動 `use_closure_phases=True`；
`nact=2`、`nlive=250`、`dlogz=0.1`、`bound='live'`、`sample='rwalk'` 全部不變。
逐筆配對比較（方法論同 X.21 / X.22 / X.24 / X.33）。

## X.34.2 行為驗證：closure 項真的進到 likelihood 裡

**沒有只看 kwargs 字典。** 開跑前的直接擾動測試：

| 檢查 | 結果 |
|---|---|
| `like.last_closure_config` | `{'used_closure_phase': True, 'reason': 'observation supplied closure phases', 'n_closure_phases': 21}` |
| 觀測 closure phase 加 +0.3 rad 後 Δln L（closure on） | **+0.8945** |
| 同一擾動 Δln L（closure off，amplitude-only） | **恰為 0.000e+00** |
| 每次呼叫成本 | 2.07 ms → 2.28 ms（1.10×） |

amplitude-only 臂對 closure phase 的擾動**完全無反應**，
closure 臂有反應——這是行為證據，不是旗標回音。
18 筆存檔記錄中 `use_closure_phases=True`、`n_baselines=28`、
`closure_is_real=True` 全部一致；15 筆收斂者的
`closure_config` 全為 `n_closure_phases=21` 的同一組值。

## X.34.3 成本估算錯了兩次（如實記錄）

事前我用**每次呼叫**的成本倍率（1.10×）推得 18 筆約 3.1 h。
**這個推法是錯的**，而且是 X.33 同一類錯誤（1.44× per-call 對上 2.08× wall）：
closure 項同時改變**取樣器需要的迭代次數**，不只是單次呼叫成本。
實測逐筆 wall ratio 從 **0.52×** 到 **≥26.6×**（被 cap 截斷者為下界），
我在跑的過程中兩次回報了這個更正。

實際結果：**15 筆收斂、3 筆撞到 3000 s 上限**，總計約 5.4 h（含被 cap 的三筆）。

## X.34.4 cap 率本身就是一個發現

| | amplitude-only | closure |
|---|---|---|
| 收斂 | 18/18 | **15/18** |
| 撞上限（3000 s） | **0** | **3**（idx 79, 52, 61） |
| cap 率 | 0.000 | **0.167**，95% CI [0.036, 0.414] |

被 cap 的三筆 SNR 為 **5.4 / 8.0 / 26.1**，全部落在低 SNR 端。
成本倍率有清楚的 SNR 結構：

| | n | wall ratio 中位數 | 被 cap |
|---|---|---|---|
| SNR < 50 | 11 | **3.04×** | 3 |
| SNR ≥ 50 | 7 | **0.81×**（反而更快） | 0 |

**這三筆不能直接丟掉。** 它們很可能正是 closure 項改變最多的那幾筆，
把它們剔除會產生跟 X.29 收斂選擇效應**同一種**的偏差。
下面同時給出剔除後的配對結果**與**把它們算進去的上下界。

## X.34.5 配對比較（15 筆收斂配對，90 組 (注入, 參數)）

### 區間寬度

| 讀數 | 值 |
|---|---|
| 90% 區間寬度比中位數（closure / amplitude） | **0.9673** |
| 平均 | 0.8873，95% CI [0.8282, 0.9464] |
| Wilcoxon（全部 90 組，log 寬度比） | **p = 0.0012** |

逐參數 Wilcoxon：`i` p = 0.0151、`position_angle` p = 0.0125，
其餘四個參數 p = 0.25–0.64。

**closure phase 讓後驗變窄，不是變寬。**

### coverage 與 McNemar

| 參數 | cov90 amp | cov90 closure | cov68 amp | cov68 closure |
|---|---|---|---|---|
| M | 0.667 | 0.533 | 0.667 | 0.467 |
| a_star | 0.800 | 0.800 | 0.533 | 0.533 |
| i | 0.733 | 0.600 | 0.400 | 0.400 |
| position_angle | 0.667 | 0.467 | 0.467 | 0.200 |
| ring_width_frac | 0.933 | 0.800 | 0.733 | 0.533 |
| log10_total_flux_jy | 1.000 | 0.867 | 0.933 | 0.867 |
| **平均** | **0.800** | **0.678** | **0.622** | **0.500** |

**沒有任何一個參數的 coverage 變好。**

| McNemar | 不一致格數 | miss→hit | hit→miss | p |
|---|---|---|---|---|
| 90% | 15 / 90 | 2 | **13** | **0.007** |
| 68% | 15 / 90 | 2 | **13** | **0.007** |

### 所需增寬（對照 MC 有限-L 期望值 0.8825 / 0.6671）

| 臂 | cov90 | cluster-bootstrap 95% CI | 所需增寬 |
|---|---|---|---|
| amplitude（全 18 筆） | 0.787 | [0.731, 0.843] | +25.7% |
| amplitude（配對 15 筆） | 0.800 | [0.744, 0.867] | +22.1% |
| **closure（配對 15 筆）** | **0.678** | [0.556, 0.800] | **+58.1%** |

68% 水準同向：amplitude 0.622（+9.8%）→ closure 0.500（**+43.6%**）。

配對子集本身沒有偏差：amplitude 臂在全 18 筆與配對 15 筆上的
coverage 差 +0.013（90%）、+0.030（68%）。

### 把被 cap 的三筆算進去的上下界

| | 最樂觀（三筆全中） | 最悲觀（三筆全失） | 僅收斂者 |
|---|---|---|---|
| cov90 | 0.731 | 0.565 | 0.678 |
| cov68 | 0.583 | 0.417 | 0.500 |

**即使最樂觀的界 0.731 仍低於 amplitude-only 的 0.787 / 0.800。**
結論不依賴如何處理被 cap 的三筆。

## X.34.6 一句話結論

**closure phase 不但沒有縮小 +9.1%～+11.8% 的殘留缺口，反而讓 coverage 明顯變差**
（90% 0.800 → 0.678、68% 0.622 → 0.500，McNemar 13 筆由中變失對上 2 筆由失變中、
p = 0.007；90% 區間寬度中位數變窄至 0.9673、Wilcoxon p = 0.0012；
最樂觀的 cap 處理下仍是 0.731 < 0.787），
**因此與 nact / nlive / dlogz 一樣被排除為缺口的解方**，
差別在於前三者是「沒有影響」、closure phase 是「有影響但方向相反」。

依要求，**不因此提出任何新的候選機制**。
既然 closure phase 沒有幫助，也**不**給完整規模 N = 100 的估算——
那份估算只在它有幫助時才有意義。

## X.34.7 限制與未追查事項

1. **n = 15 配對（18 筆中 3 筆被 cap）**，cluster-bootstrap CI 仍寬
   （closure cov90 [0.556, 0.800]）。本節能斷定方向與顯著性，
   不能精確定出變差的幅度。
2. **為什麼 closure phase 會讓區間變窄且 coverage 變差，本節沒有追查。**
   一個未檢驗的可能是 closure phase 與 amplitude 在同一個 likelihood 裡
   重複計入了相同的資訊（closure phase 由同一組可見度導出，並非獨立量測），
   但**本節沒有做任何檢驗來支持或排除這一點，僅登記為未測候選。**
   依指示不往下猜。
3. 三筆被 cap 的注入沒有跑到收斂，它們的後驗形狀未知。
4. 本 pilot 只在 M87\*，未在 Sgr A\* 重複。

## X.34.8 產出

- `artifacts/image_sbc_closure/M87star/`（18 筆：15 收斂 / 3 撞上限）
- `artifacts/image_sbc_closure/paired_vs_uv28.csv`
- `docs/calibration/image_gr_eternal_closure_pilot.csv`（同一份，歸檔在 calibration 下）

---

# X.35 `bh_accretion` 在 M87* 上的首次 SBC/coverage 校準 —— **pilot 階段中止，回報成本與取樣偏差**

**這是校準驗證，不是任何白洞結論。** WhiteSearch 是候選排序引擎。
**完整規模 N=100 未開跑**，原因見 X.35.5，依指示先回報。

## X.35.1 取樣維度與參數（要求 1，以程式碼實際狀態為準）

`bh_accretion` 取樣 **6 個參數**：

| 參數 | 先驗 | 範圍 |
|---|---|---|
| `M` | log_uniform（逐目標） | 3.0e9 – 1.0e10 M☉ |
| `a_star` | uniform | 0 – 0.998 |
| `i` | cos_uniform | 0 – π |
| `position_angle` | uniform | 0 – π |
| `log10_mdot_edd` | uniform | −5 – 0 |
| `jet_power_frac` | log_uniform | 0.01 – 1.0 |

- **`D_L` 已是逐目標 context，不是取樣參數**（`requires_target = True`，
  距離自 `utils.targets` 讀），與 `gr_eternal` 一致。
- **`model.parameters()` / `likelihood.parameter_names` /
  `effective_parameter_names` 三者完全一致**，沒有「宣告了但沒被取樣」的參數。
  `gr_eternal` 是「宣告 8、有效 6」（`log10_ne` / `log10_B` 為死參數），
  `bh_accretion` 是「宣告 6、有效 6」。
- **逐參數活性已用擾動重新確認（教訓 1）**：三組先驗抽樣、每個參數 ±2% / ±10%，
  **六個參數的 ΔlnL 全部非零**，沒有任何一個恰為 0。
  bilby 自己的 priors 日誌也列出同樣的六個，這是取樣器端的獨立確認。

## X.35.2 先驗合理性（要求 2）

X.16.4 做過這個檢查，但跑在**舊的 16 條基線**上。以現行程式碼（28 條基線）
重跑，N=400 先驗預測，M87\*：

| | `bh_accretion` | `gr_eternal` |
|---|---|---|
| 總流量 5/50/95% | 0.0080 / 0.609 / 84.2 Jy | 0.070 / 0.783 / 8.57 Jy |
| 流量跨度（5–95%） | **4.02 dex** | 2.09 dex |
| SNR 5/50/95% | 0.54 / 41.8 / **4690** | 4.19 / 45.7 / 507 |
| SNR > 1000 比例 | **20.3%** | **0.0%** |
| SNR < 1 比例 | **11.3%** | 0.0% |
| 落在實測 0.5–1.2 Jy 的先驗質量 | 0.087 | 0.188 |

實際要跑的 100 筆真值（seed base 710000）SNR **0.047 – 9959，橫跨 5.3 dex**；
10 筆 SNR < 1、17 筆 SNR > 1000。不可表示的環 0/100。

**評估結果：參數化對症，先驗未收窄，理由三點。**

1. **不能照 `gr_eternal` 的方式修。** X.16.4 的論證經本輪重新確認：
   `log10_mdot_edd` 同時決定亮度**與**厚度，這個耦合就是這個假說的實質內容
   （X.12.3）；改成取樣總流量會把亮度那一半歸一化掉，`bh_accretion` 會退化成
   「厚度由單一參數決定的 `gr_eternal`」。對它對症的是重錨 `LOG10_I0_EDD`，
   已於 X.13.5 完成。
2. **先驗寬不影響 SBC 的有效性。** SBC 是自洽檢驗：真值自先驗抽、擬合用同一
   先驗。先驗寬只影響成本與代表性，**不造成 coverage 偏差**。
   SNR < 1 的那 10 筆後驗≈先驗、rank 依構造均勻，是正確行為而非 bug。
3. **真正的風險是成本**，而成本用 pilot 實測，不外推猜（教訓 6）。

## X.35.3 Pilot（要求 3）

20 筆、依 SNR 分層（每個 SNR 數量級 4 筆），
`nact=2` / `nlive=250` / `dlogz=0.1` / `cap=3000 s`，28 條基線、amplitude-only。

**結果：20 筆中 13 筆收斂、7 筆撞 3000 s 上限，共 9.27 h。**

| SNR 區間 | pilot 筆數 | 被 cap | cap 率 | 平均牆鐘 |
|---|---|---|---|---|
| [0, 1) | 4 | 0 | 0.00 | 536 s |
| [1, 10) | 4 | 0 | 0.00 | 725 s |
| [10, 100) | 4 | 0 | 0.00 | 1471 s |
| **[100, 1000)** | 4 | **3** | **0.75** | 2613 s |
| **[1000, ∞)** | 4 | **4** | **1.00** | 3000 s |

**cap 完全由 SNR 決定**：SNR < 100 的 12 筆**全部收斂**，
SNR ≥ 100 的 8 筆**有 7 筆被 cap**（唯一例外 idx 16，SNR 197.8，1451 s）。

## X.35.4 與 `gr_eternal` 的成本比較（同 SNR 區間，皆為收斂者）

| SNR 區間 | `gr_eternal` n / 中位牆鐘 | `bh_accretion` n / 中位牆鐘 | 比值 |
|---|---|---|---|
| [1, 10) | 22 / 65 s | 4 / 686 s | **10.6×** |
| [10, 100) | 44 / 177 s | 4 / 1385 s | **7.8×** |
| [100, 1000) | 29 / 571 s | 1 / 1451 s | **2.5×** |

**這個比較被 uv 覆蓋的改變混淆，必須說明**：`gr_eternal` 的存檔 campaign
跑在 16 條基線上，`bh_accretion` 跑在 28 條上，而 X.33 量到覆蓋改變本身
就值 **2.08×**。扣掉之後仍有約 **1.2× – 5.1×**，
所以「`bh_accretion` 的 likelihood 曲面對取樣器更難」這一點**方向成立，
但倍率沒有被乾淨地隔離出來**——本輪沒有跑同覆蓋的對照，不宣稱精確倍率。

## X.35.5 為什麼 N=100 沒有開跑（要求 3 的成本閘門）

兩個獨立的理由，任一個都足以中止。

### (1) 耗時遠超 `gr_eternal` 的量級

用 pilot 的**逐 SNR 區間實測 cap 率與平均牆鐘**外推那 100 筆真值：

| 估法 | N=100 耗時 | 預測被 cap |
|---|---|---|
| 逐區間實測（採用） | **47.6 h** | **33/100** |
| pilot 平均直接放大 | 46.4 h | — |
| 擬合 `wall = 629 × SNR^0.180`（**不採用**） | 39.1 h | 3/100 |
| 參考：`gr_eternal` N=100 實績 | **10.23 h** | 5/100 |

**擬合法被否決，理由要記下來**：它只用得上**收斂**的 13 筆，
而被 cap 的 7 筆正是最貴的那一端——這是右設限資料。
擬合因此預測 3/100 被 cap，對上 pilot 實際的 7/20（35%）。
這與 X.31 成本模型低估 23%、預測 0 筆 cap 對上實際 1 筆是**同一類錯誤**，
也是教訓 6 的第四次實例。

**47.6 h 是 `gr_eternal` 的 4.7 倍**，遠超要求 3 所指的「比照 gr_eternal 規模」。

### (2) 更嚴重：cap 會造成一個乾淨的高 SNR 截斷，量出來的 coverage 不可解讀

預估 **33/100 被 cap，而且全部來自 SNR ≥ 100 那一端**。
`gr_eternal` 的 5/100 不是乾淨切口（SNR 140 被 cap、SNR 586 收斂），
X.29 才能用算術上界證明它最多只佔缺口的 18%（I-6m）。
這裡是**依 SNR 單調的截斷**：SNR < 100 全收斂、SNR ≥ 100 幾乎全被切掉。

在這種取樣下算出來的 coverage，**測到的是「低 SNR 子母體的 coverage」**，
不是這個先驗下的 coverage。這正是候選 9（收斂選擇效應）的機制，
量級卻比 `gr_eternal` 當初面對的大一個數量級。
**先跑完再事後修正是行不通的**：被切掉的那一端沒有後驗，
只能給界，而 33% 的界寬到沒有判定力。

**依要求 5/6，本輪到此回報並停止，不自行選一種修法。**

## X.35.6 13 筆收斂者的初步讀數 —— **不是判定**

記錄下來供參考。**這 13 筆的 SNR 分布被 cap 系統性偏向低端
（最高收斂 SNR 197.8，8 筆 SNR ≥ 100 中 7 筆缺席），
所以下表不能當成 `bh_accretion` 的 coverage 判定，
它正是 X.35.5(2) 所說的那種有偏子樣本。**

| 參數 | rank 平均 | rank sd | 極端 rank | KS p | 離散虛無對照 | chi² p | 90% cov | 68% cov |
|---|---|---|---|---|---|---|---|---|
| `M` | 54.0 | 33.5 | 0.077 | 0.8333 | 0.845 | 0.7166 | 0.769 | 0.615 |
| `a_star` | 54.6 | 28.4 | 0.000 | 0.9015 | 0.917 | 0.8603 | 1.000 | 0.615 |
| `i` | 55.9 | 27.8 | 0.077 | 0.8293 | 0.848 | 0.2864 | 0.923 | 0.692 |
| `position_angle` | 50.0 | 28.2 | 0.000 | 0.9362 | 0.936 | 0.5576 | 0.923 | 0.692 |
| `log10_mdot_edd` | 38.8 | 36.5 | **0.308** | 0.0516 | 0.055 | 0.0790 | 0.615 | 0.385 |
| `jet_power_frac` | 34.7 | 30.2 | 0.000 | 0.1670 | 0.154 | 0.4094 | 0.769 | 0.615 |

理論值：rank 平均 50.0、sd 29.15、極端 rank 比例 0.0198；
Bonferroni 門檻 p > 0.0083。

| 彙總（N=13，**有偏子樣本**） | 值 |
|---|---|
| 90% coverage | 0.8333，95% CI [0.7308, 0.9231]，bootstrap p = **0.192** |
| 68% coverage | 0.6026，95% CI [0.4872, 0.7179]，bootstrap p = **0.127** |
| 所需增寬 | +13.2%（CI 0–41.7%）／+14.4%（CI 0–48.0%） |

**兩個水準都不顯著**（p = 0.192 / 0.127）。**但 N=13 的檢定力本來就不足以
分辨 +11.8% 量級的缺口**——這不是「校準良好」的證據，是「還沒測」。
依教訓 5，顯著性檢定已與 rank 報告同一趟做完，沒有拖到事後。

**未追查、只登記**：`log10_mdot_edd` 的極端 rank 比例 0.308（理論 0.0198）
明顯偏高，且 rank 平均 38.8 偏低。N=13 下這可能只是雜訊
（KS p = 0.0516 通不過 Bonferroni 0.0083），**本輪不對它提出任何機制，
也不追查**，等完整規模的資料再看。

## X.35.7 教訓 3 的對照組：KS 檢定在 N=13 / N=95 下都是校準的

每個 KS p 都附了一個「精確離散虛無分布」對照（ranks ~ Uniform{0..L}，
同 N、同統計量，2000 次）。上表的 KS p 與對照分位**逐列吻合**
（0.8333/0.845、0.9015/0.917、0.0516/0.055…），
在已存檔的 `gr_eternal` N=95 上也吻合（0.8353/0.851、0.0014/0.002）。
**X.30 那種「離散 rank 套連續檢定產生假顯著」的假象在這個 N 不出現**，
它是大 N 才發生的。這個對照現在寫死在 `analyse_image_sbc.py` 裡，
不必每次重新論證。

## X.35.8 產出

- `artifacts/image_sbc_bha_pilot/M87star/`（20 筆：13 收斂 / 7 撞上限）
- `docs/calibration/image_bh_accretion_pilot.csv`
- `scripts/analyse_image_sbc.py`（模型無關的 X.20 格式報告，新）
- `scripts/run_image_sbc.py` 的 `--model` / `--seed-base`

## X.35.9 尚待決定（不自行決定）

完整規模要怎麼跑，有幾個方向會改變不同的東西，**等決定**：
提高 cap、收窄 `log10_mdot_edd` 先驗、接受設限並只給界、或投入更多算力。
**本輪不選、不預設，也不往下猜新的候選機制。**

---

# X.36 `bh_accretion` 高 SNR 端收斂性補測

**這是校準診斷，不是任何白洞結論。** WhiteSearch 是候選排序引擎。
**完整規模 N=100 仍未開跑**，依指示先回報。

## X.36.1 為什麼做這件事

X.35 的 pilot 顯示 cap 率隨 SNR **單調上升**
（[0,100) = 0/12、[100,1000) = 3/4、[1000,∞) = 4/4），
外推到完整規模預估 33/100 被 cap，而且是**乾淨的 SNR 單調截斷**。
用 3000 s 硬跑完整規模，量到的會是「跑得完的那個子集」的 coverage。

本輪**只補測高 SNR 端**，不整組重跑、不收窄先驗。

## X.36.2 timeout 的依據（不是猜的）

X.35 只能事後手動讀 checkpoint 才知道被 cap 的那幾筆離收斂多遠。
先把那七筆的 dynesty 狀態讀出來：

| idx | SNR | it@3000s | eff | `dlogz_remaining` |
|---|---|---|---|---|
| 25 | 106.3 | 3515 | 0.91% | 6.52 |
| 73 | 489.7 | 6106 | 2.03% | **0.607** |
| 43 | 891.2 | 7059 | 2.14% | **0.650** |
| 39 | 1120.4 | 4280 | 1.16% | **186.9** |
| 85 | 1486.2 | 5866 | 1.64% | 40.94 |
| 7 | 4855.4 | 7643 | 2.17% | 49.31 |
| 17 | 9959.4 | 7446 | 2.11% | **109.3** |

dynesty 每次迭代把先驗體積縮約 1/`nlive`，所以從 `dlogz_now` 走到 0.1
大約還需要 `nlive × ln(dlogz_now / 0.1)` 次迭代——換算成**已跑迭代數的
1.07–1.44 倍**。

**選 12000 s（4×）**，依據：對最壞情況（1.44×）留 **2.78 倍**餘裕，
足以吸收 checkpoint 裡看到的效率衰退（eff 已掉到 0.91–2.17%，
即每個接受點要花 46–110 次 likelihood 呼叫）；
最壞情況總預算 9 × 12000 s = 30 h，量得起。
6×（18000 s）只是對「`logl_max` 仍在爬升」這個失效模式多留餘裕，
而那個模式若成立，多 1.5 倍也解決不了，卻把最壞預算推到 45 h。

**當時已標註的保留**：上面的迭代數估計假設 `logl_max` 停止爬升。
極高 SNR 下峰很窄、`logl` 會持續上升，`dlogz_remaining` 可能長時間居高不下，
所以 1.07–1.44× 是**下界**。

## X.36.3 樣本

pilot 被 cap 的 7 筆，加上 2 筆**先前未跑過**的抽樣
（idx 54 SNR 697、idx 68 SNR 2702，兩個區間各一），共 **9 筆**——
避免 cap 的樣態只從 pilot 那幾個特定抽樣讀出來。
其餘設定完全不變（`nact=2` / `nlive=250` / `dlogz=0.1`、28 條基線、amplitude-only）。

## X.36.4 結果：**9/9 全部收斂，0 筆被 cap**

| idx | SNR | `dlogz`@3000s | 補測耗時 | ÷3000s | 狀態 |
|---|---|---|---|---|---|
| 25 | 106.3 | 6.52 | 4223 s | 1.41 | ok |
| 73 | 489.7 | 0.607 | 2439 s | 0.81 | ok |
| 54 | 697.3 | （新抽） | 1471 s | 0.49 | ok |
| 43 | 891.2 | 0.650 | 2292 s | 0.76 | ok |
| 39 | 1120.4 | 186.9 | 7315 s | 2.44 | ok |
| 85 | 1486.2 | 40.94 | 4272 s | 1.42 | ok |
| 68 | 2702.4 | （新抽） | 5637 s | 1.88 | ok |
| 7 | 4855.4 | 49.31 | 4912 s | 1.64 | ok |
| 17 | 9959.4 | 109.3 | 7624 s | 2.54 | ok |

**最長的一筆 7624 s，只用掉 12000 s 預算的 64%。**

**`dlogz_remaining` 準確預測了需要多少時間**：
log10(`dlogz`@3000s) 與 log10(補測耗時) 的相關 **r = 0.959**（n = 7）。

### 對要求 4 的回答：**是預算不夠，不是結構性困難**

沒有任何一筆在 4 倍預算下撞上限，而且最貴的一筆離上限還有 36% 餘裕。
X.36.2 裡標註的「`logl_max` 持續爬升可能使 `dlogz` 居高不下」這個疑慮
**沒有發生**——連 `dlogz_remaining` 高達 186.9 的 idx 39 也在 7315 s 收斂。
**因此不需要提出任何機制去解釋「結構性困難」，因為沒有觀察到結構性困難。**

## X.36.5 一個必須回報的量測疑慮：兩次跑的吞吐量差 1.49 倍

同樣 7 筆真值、同樣設定，pilot 與補測的 likelihood 呼叫速率不同：

| | pilot | 補測 | 比值 |
|---|---|---|---|
| calls/s | 100–128 | 149–214 | **1.40–1.67，中位數 1.49** |

**這是量測條件的差異，不是注入的性質。**
最可能的候選是 pilot 進行期間容器裡同時跑了其他工作（測試套件、先驗掃描）
造成 CPU 競爭，resume 開銷是另一個候選。
**本輪沒有做任何檢驗去分辨，僅回報為不確定性，不診斷成因。**

影響：pilot 當時的 3000 s 上限，有效取樣時間只相當於補測的約 2000 s。
這表示 **X.35 量到的 33/100 cap 率本身偏高**，但方向不變
（cap 仍隨 SNR 單調上升），而本輪的結論（4× 下 0/9）不受影響。

## X.36.6 重新擬合成本（要求 3）

**censoring 已經消失，不是被處理掉。** X.35 的擬合錯在只用收斂樣本，
而最貴的 7 筆被右設限在 3000 s。補測之後 **22 筆相異注入全部收斂**，
所以這裡用一般擬合是正當的——不是因為設限問題不重要，是因為設限本身沒了。
同一筆跑過兩次時**只取收斂的那次**；被 cap 的那次是同一個量的下界，
拿去擬合會把結果壓低。

**選擇偏差稽核**：pilot 每個 SNR 數量級隨機抽 4 筆，那 20 筆**現在全部都有量測值**
（7 筆被 cap 的已在補測收斂），所以逐區間樣本是**完整的、沒有依成本篩選過**。
另外 2 筆是按 SNR 中位數挑的，是對 SNR 的選擇，不是對成本的選擇。

| 擬合 | 結果 |
|---|---|
| 全區間（n=22，SNR 0.047–9959，5.3 dex） | `wall = 611 × SNR^0.247`，殘差 sd 0.182 dex（×1.52） |
| 只取 SNR ≥ 1（n=18） | `wall = 507 × SNR^0.279`，殘差 sd 0.190 dex |
| 參考：`gr_eternal` | `wall ∝ SNR^0.56` |

**指數比 `gr_eternal` 平緩**——`bh_accretion` 在低 SNR 貴得多，但隨 SNR 上升得比較慢，
與 X.35.4 量到的「[1,10) 差 10.6×、[100,1000) 只差 2.5×」一致。

### 分層預算下的 N=100 估計

策略：**SNR < 100 用 3000 s 上限、SNR ≥ 100 用 12000 s 上限**。

| SNR 區間 | N=100 筆數 | 已量測 | 平均耗時 | 估計 |
|---|---|---|---|---|
| [0, 1) | 10 | 4 | 536 s | 1.5 h |
| [1, 10) | 22 | 4 | 725 s | 4.4 h |
| [10, 100) | 30 | 4 | 1471 s | 12.3 h |
| [100, 1000) | 21 | 5 | 2375 s | 13.9 h |
| [1000, ∞) | 17 | 5 | 5952 s | 28.1 h |
...
| 冪律擬合 | 54.7 h |
| 擬合 +1sd | 83.3 h |
| 低三區間除以 1.49 的吞吐量修正（X.36.5） | **54.2 h** |
| 參考：`gr_eternal` N=100 實績 | **10.23 h** |

**預測被 cap：0/100（+1sd 也是 0/100）。**
分層預算**消除了那個 SNR 單調截斷**——這是本輪最重要的實用結論。

**代價是總耗時約 54–60 h，是 `gr_eternal` 的 5.3–5.9 倍。**
這是不是「離譜地更貴」由你判斷；**依要求 5，本輪不開跑。**

## X.36.7 附帶：把截斷移除後，N=22 的 coverage 讀數

**不是判定，N=22 檢定力仍不足。** 但它直接展示了截斷的方向。

| | N=13（X.35，被截斷、偏低 SNR） | **N=22（本輪，無截斷）** | 參考 |
|---|---|---|---|
| 90% coverage | 0.8333，p = 0.192 | **0.8788**，95% CI [0.803, 0.947]，p = 0.444 | 0.8825 |
| 68% coverage | 0.6026，p = 0.127 | **0.6667**，95% CI [0.568, 0.758]，p = 0.463 | 0.6671 |
| 所需增寬 | +13.2% / +14.4% | **+1.0% / +0.1%** | — |

按 SNR 三分位拆解（教訓 2），方向很清楚：
...
**被 3000 s 上限切掉的，正是 coverage 最好的那些筆。**
所以 X.35 那個 0.8333 偏低，是截斷造成的，方向與當時的擔憂一致。

**必須講清楚的保留**：
1. **N=22 不足以宣稱「校準良好」。** 90% 的 CI 是 [0.803, 0.947]，
   既相容於參考值，**也仍相容於高達 +21% 的缺口**。
2. 因此**跨模型交叉驗證（`gr_eternal` 的 +9.1%~+11.8%）本輪仍無法作答**。
3. 這 22 筆是 SNR 分層抽樣，**不是先驗的隨機樣本**，
   逐區間權重與完整 N=100 不同，彙總數字不能直接當成先驗平均 coverage。
4. `log10_mdot_edd` 的極端 rank 比例從 0.308 降到 **0.227**（理論 0.0198），
   仍然偏高。**本輪不對它提出任何機制，也不追查**，只登記。

## X.36.8 工具鏈

被 cap 的注入現在會把 dynesty 內部狀態寫進記錄
（`it` / `ncall` / `eff` / `nbound` / `logz` / `dlogz_remaining` /
`logl_max_live`），不必再事後手動讀 checkpoint。
**本輪 0/9 被 cap，所以這段程式沒有在正式執行中被觸發過**——
已另外對 pilot 的 7 個 checkpoint 直接呼叫驗證（含 checkpoint 不存在的分支）。

## X.36.9 產出

- `artifacts/image_sbc_bha_highsnr/M87star/`（9 筆，9/9 收斂，11.16 h）
- `docs/calibration/image_bh_accretion_highsnr_retest.csv`
- `docs/calibration/image_bh_accretion_n22_uncensored.csv`

---

# X.37 pilot／補測吞吐量差異診斷

**這是環境／效能診斷，不含任何科學結論，也沒有跑任何 SBC campaign。**

## X.37.1 要回答的問題

X.36.5 回報了一個未解釋的量測差異：同樣的注入、同樣的設定，
pilot 量到 **100–128 calls/s**、補測量到 **149–214 calls/s**，差 **1.49 倍**。
當時唯一的候選解釋是「pilot 期間容器內有其他工作造成 CPU 競爭」，未經檢驗。
決定要不要花約 60 h 跑完整 campaign 之前，需要知道**這件事會不會再發生**。

## X.37.2 方法

`scripts/diagnose_throughput.py` 直接量 likelihood 的呼叫速率——
**不經過取樣器、不經過 bilby**，所以被計時的只有前向模型。
取樣器開銷是上面的一個小常數，造不出 1.49 倍的擺動。
呼叫點取自先驗抽樣（不是重複呼叫真值），因為成本隨參數而變。
同時採樣 `/proc/stat` 的 **steal time**：這是 Firecracker microVM，
宿主機繁忙會表現為 steal，而那是本專案裡任何程式都修不掉的成因。

環境：**4 顆 CPU、cgroup 無配額限制**（`cpu.max` = `-1 100000`）。

## X.37.3 結果：根因確認

| 條件 | calls/s | 對乾淨環境的比值 |
|---|---|---|
| 乾淨 | **146.3**（145.9–146.6） | 1.00 |
| 乾淨（重複量測） | **146.5**（144.8–147.0） | 1.00 |
| 1 個背景行程（4 核佔 1 核） | 144.8 | **1.01** |
| **CPU 飽和**（4 核上有 5+ 個可執行行程） | **98.4**（89.0–100.9） | **1.49** |

**實測的 1.49 倍與 pilot／補測觀察到的 1.49 倍完全吻合。**
乾淨值 146 落在補測區間（149–214）的低端，飽和值 98 落在 pilot 區間（100–128）的低端。
乾淨基準可重現（146.3 / 146.5），效應可逆。

**關鍵的細節：單一背景任務幾乎沒有影響（1.01 倍）。**
要看到 1.49 倍，CPU 必須是**超額訂閱**（可執行行程數 > 核心數），
不是只有「背景有東西在跑」。
...
| 量測 | 使用核心數 |
|---|---|
| numpy BLAS matmul（1500×1500） | **3.92** |
| `VisibilityLikelihood.loglike` | **1.00** |

`OMP_NUM_THREADS` / `OPENBLAS_NUM_THREADS` / `MKL_NUM_THREADS` **都沒有設定**，
所以 BLAS 不受限地用滿 4 核，而**取樣器本身是單執行緒的**（影像/FFT 工作，不是 BLAS）。

**因此一支看起來無害的分析腳本就足以讓整台機器飽和。**
pilot 期間跑過的東西正是這一類：先驗掃描、逐參數活性測試、
`analyse_image_sbc.py`（200000 次 bootstrap ＋ 每參數 2000 次離散虛無對照）、
以及兩次完整測試套件。補測期間則大多是等待。

steal time 全程都在 **0.17–0.87%**，所以**宿主機競爭不是這次的機制**，
是 VM 內部的超額訂閱。

## X.37.4 checkpoint 開銷：會成長，但不是成因（要求 3）

| | 值 |
|---|---|
| resume 檔案大小 | 0.65 → **3.28 MB**（與牆鐘時間相關 r = 0.805） |
| `check_point_delta_t` | 45 s |
| 最大檔案寫一次實測 | **226 ms** |
| 佔 45 s 間隔的比例 | **0.503%** |
| 12000 s 一輪的總寫入量 | 約 0.88 GB |

**檔案確實隨迭代數成長，但單次寫入只佔牆鐘的 0.5%。**
要成為 1.49 倍的成因，它得再長 100 倍。**排除。**

## X.37.5 一句話結論（要求 4）

**根因已確認：是 VM 內 CPU 超額訂閱造成的，屬於「一次性的環境巧合」而非
長時間 campaign 中會自行反覆出現的系統性風險**——
只要 campaign 執行期間不在同一個容器裡跑其他 CPU 密集工作就不會發生
（checkpoint 開銷這個會隨時間累積的候選已量測排除，僅佔 0.5%）。

## X.37.6 保留與尚待決定

1. **機制已確認，但無法回溯證明 pilot 當時確實整段都處於飽和。**
   沒有當時的逐時刻 CPU 記錄。證據是情境性的：飽和實驗精確重現了 1.49 倍，
   而 pilot 期間在跑上述那些工作、補測期間大多在等待。
   pilot 觀察到的速率有 100–128 的散布（1.28 倍），與「間歇性競爭」相符。
2. **「可避免」這件事比聽起來微妙**：因為 BLAS 執行緒數無上限，
   **單一一支分析腳本就能佔滿 4 核**。不是只要「不要跑很多東西」，
   而是 campaign 期間**任何** numpy 密集的工作都要避開。
   把分析腳本的 BLAS 執行緒數限制成 1 是一個選項，
   **本輪未實作，依指示不自行決定。**
3. **耗時估計模型本輪未調整**，依指示等你評估。
   參考：X.36 的 N=100 估計 54.2–60.1 h，其中低三個 SNR 區間來自 pilot 計時
   （可能受此效應影響而偏高），高兩個區間來自補測（乾淨）。
...
# X.37 更正（campaign 進行中發現）：吞吐量差異**不只是**可避免的 CPU 競爭

**X.37 的一句話結論需要修正。** X.37 說 pilot／補測的 1.49 倍差異是
「一次性的環境巧合，不是長時間 campaign 會自行反覆出現的系統性風險」。
N=100 campaign 跑了 11 小時後的資料不支持這個說法。

## 新證據

| 量測 | calls/s | 條件 |
|---|---|---|
| 補測（9 筆，整段取樣迴圈） | 中位數 **172**（148–230） | 無 BLAS 保護、無競爭 |
| **campaign 新跑（3 筆，整段取樣迴圈）** | 中位數 **115**（113–128） | 有 BLAS 保護、**無競爭** |
| pilot（7 筆） | 100–128 | 無保護、有競爭 |

在**相同 SNR 範圍**，campaign 三筆全部慢於補測九筆，比值又是 **1.49 倍**——
但這次**沒有競爭**：load 1.0、只有 campaign 一個行程佔一核、閒置 74%、steal 0.27%。

## 兩個候選的排除與量測

1. **BLAS 保護（X.38）：排除。** X.38 的 A/B 只測了裸 likelihood，
   沒測完整取樣迴圈——這是當時驗證的缺口。本次補做：同一注入（idx 73）、
   同一 seed、各跑 240 s，**1 條 BLAS 執行緒 93.3 calls/s、3 條 92.8 calls/s**，
   完全相同。X.38「保護不拖慢 campaign」的結論成立。
2. **主機速度：解釋約 10%。** 本容器（Intel Xeon @ 2.80 GHz）跑 X.37 同一支
   裸 likelihood 基準得 **132 calls/s**，X.37 當時的容器是 **146**。
   容器重啟會換到不同實例，速度不同。

## 修正後的判讀

- X.37 **直接量到**「CPU 超額訂閱會造成 1.49 倍減速」——這一點仍然成立。
- 但 X.37 **推斷**這就是 pilot 慢的原因，當時已標註「無法回溯證明」。
  現在 campaign 在**無競爭**下跑出與 pilot 同一個速率，
  **較簡約的解釋是：補測那次落在一台比較快的主機上，pilot 的速率才接近常態**。
- 因此吞吐量**會隨容器實例變動**，而長時間 campaign 中容器會重啟、換實例——
  **這是會反覆出現的系統性變異，不是只要避開背景工作就能消除的一次性巧合。**
  X.37 對此的結論**錯了**。
- 這不影響 SBC 的有效性（dynesty 給定 seed 是決定性的，速度只改變牆鐘），
  **但會影響是否撞上限**：idx 0（SNR 7029）在 12000 s 上限被 cap，
  當時 `dlogz_remaining` = **0.145**（目標 0.1），差一點就收斂。
  以目前速度，高 SNR 端可能再出現幾筆 cap，**重新引入分層預算原本要消除的
  SNR 單調截斷**。被 cap 的注入都有 checkpoint，可從原處續跑。

## 耗時估計修正

X.36 的高 SNR 區間用的是補測（快主機）的計時，需乘 1.49；
低 SNR 區間用的是 pilot 計時，pilot 速率與 campaign 相近，不調整。
78 筆新跑原估 **46 h → 約 62 h**（已跑 11 h、剩約 51 h），
仍在「超過 80–90 h 才中途回報」的門檻內，campaign 繼續執行。
**上限政策本輪未更動**，是否提高高 SNR 端上限等你決定。
